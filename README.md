...
4. After clicking the desired workflow run, you should be presented with a section at the bottom of the page called "Artifacts". Download the artifact named `glove80.uf2`; it contains the firmware file and `build-metadata.txt`, which records the exact `moergo-sc/zmk` commit used for the build.
5. Extract `glove80.uf2` from the downloaded artifact.
6. Flash the firmware to Glove80 according to the user documentation on the official Glove80 Support website (linked above).
